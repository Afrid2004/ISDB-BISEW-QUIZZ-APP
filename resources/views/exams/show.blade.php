        ...
        <div class="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-xl bg-white shadow-xl">
            ...
            <div class="max-h-[calc(90vh-130px)] overflow-y-auto">
                ...
                <form action="{{ route('exams.exam-sets.store', $exam) }}" method="POST">
                    ...
                    @include('exams.exam-sets._form', ['examSet' => null])
            ...
            <div class="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-xl bg-white shadow-xl">
                ...
                <div class="max-h-[calc(90vh-130px)] overflow-y-auto">
                    ...
                    <form action="{{ route('exams.exam-sets.update', [$exam, $examSet]) }}" method="POST">
                        ...
                        @include('exams.exam-sets._form', ['examSet' => $examSet])
            ...
            <div class="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">
                ...
                <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-6">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-400">
                            Manage questions for {{ $examSet->name }}
                        </p>
                    </div>
                    ...
                    <form action="{{ route('exam-sets.questions.update', $examSet->id) }}" method="POST"
                        ...
                        @php
                            ...
                            foreach ($examSet->competencyUnitMappings as $mapping) {
                                ...
                                $moduleId = $mapping->competencyUnit->module_id;

                                if (!isset($existingMappings[$moduleId])) {
                                    $existingMappings[$moduleId] = [];
                                }

                                $existingMappings[$moduleId][$mapping->competency_unit_id] = [
                            ...
                            if ($examSet->competencyUnitMappings->count() > 0) {
                                $savedActive = (bool) $examSet->competencyUnitMappings->first()->is_active;
                            }
                        @endphp
                        ...
                        <div class="mt-6 flex items-center justify-end gap-3">
                            ...
                            <button type="submit"
                                class="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-primary/90">
                                <i class="bi bi-check-lg"></i>
                                Save Questions
                            </button>
