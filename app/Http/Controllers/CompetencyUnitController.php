...
class CompetencyUnitController extends Controller
{
                ...
                $query->where(function ($q) use ($search, $moduleNumber) {
                    ...
                    $q->orWhereHas('module', function ($moduleQuery) use ($search, $moduleNumber) {
                        ...
                        if (is_numeric($moduleNumber)) {
                            $moduleQuery->orWhere('module_number', $moduleNumber);
                        }
                        $moduleQuery->orWhereHas('course', function ($courseQuery) use ($search) {
                            $courseQuery->where('name', 'like', "%{$search}%")->orWhere('code', 'like', "%{$search}%");
                        });
                    });
                });
            })
            ->join('modules', 'competency_units.module_id', '=', 'modules.id')
    ...
    public function create()
    {
        ...
        return view('competency-units.create', compact('courses', 'modules'));
    }
    ...
    {
        $last = CompetencyUnit::where('module_id', $moduleId)->orderByDesc('serial')->first();
        $serial = $last ? $last->serial + 1 : 1;

        return response()->json(['serial' => $serial]);
    }

    // Store competency unit
    public function store(Request $request)
    {
        $request->validate([
            'prefix' => ['required', 'string', 'max:10', 'alpha'],
        ...
        $module = Module::with('course')->where('id', $request->module_id)->where('course_id', $request->course_id)->firstOrFail();
        $courseCode = strtoupper($module->course->code);
        $moduleNumber = $module->module_number;

        // Find last serial of this module
        $lastCompetencyUnit = CompetencyUnit::where('module_id', $module->id)->orderByDesc('serial')->first();
        ...
        // Generate code
        $code = strtoupper($request->prefix) . $courseCode . $moduleNumber . str_pad($serial, 2, '0', STR_PAD_LEFT);
    ...
    public function edit(CompetencyUnit $competencyUnit)
    {
        ...
        $courses = Course::where('is_active', true)->get();
        $modules = Module::where('course_id', $competencyUnit->module->course_id)->where('is_active', true)->orderBy('module_number')->get();

        return view('competency-units.edit', compact('competencyUnit', 'courses', 'modules'));
    }

    // Update competency unit
    public function update(Request $request, CompetencyUnit $competencyUnit)
    {
        ...
        if ($module->id != $competencyUnit->module_id) {
            $last = CompetencyUnit::where('module_id', $module->id)->orderByDesc('serial')->first();
            $serial = $last ? $last->serial + 1 : 1;
        } else {
        ...
        // Generate code
        $code = $prefix . $courseCode . $module->module_number . str_pad($serial, 2, '0', STR_PAD_LEFT);
    ...
    // Display deleted competency units
            ...
            ->when($search, function ($query) use ($search) {
                $query->where('code', 'like', "%{$search}%")
                    ...
                    ->orWhereHas('module', function ($q) use ($search) {
                        $q->where('name', 'like', "%{$search}%")->orWhere('module_number', 'like', "%{$search}%");
                    })
                    // Course search
