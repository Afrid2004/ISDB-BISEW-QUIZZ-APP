...
class ExamSetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $examSets = ExamSet::query()->with('exam')->when($search, function ($query, $search) {
            $query->where(function ($q) use ($search) {
                if (is_numeric($search)) {
                    $q->where('id', $search);
                }
                $q->orWhere('name', 'like', "%{$search}%")->orWhere('type', 'like', "%{$search}%")->orWhere('question_type', 'like', "%{$search}%")->orWhere('mode', 'like', "%{$search}%")->orWhere('status', 'like', "%{$search}%")->orWhereHas('exam', function ($examQuery) use ($search) {
    ...
    {
        $exams = Exam::query()->with(['course', 'batch'])->where('is_active', true)->whereNull('deleted_at')->orderByDesc('id')->get();
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            'exam_id' => ['required', Rule::exists('exams', 'id')->where('is_active', true)->whereNull('deleted_at')],
            'name' => ['required', 'string', 'max:100'],
            'type' => ['required', Rule::in(['mid', 'monthly'])],
            'question_type' => ['required', Rule::in(['mcq', 'evidence'])],
            'mode' => ['required', Rule::in(['online', 'offline'])],
            'status' => ['nullable', Rule::in(['draft', 'published', 'processing', 'completed', 'cancelled'])],
            'set_number' => ['required', 'integer', 'min:1', 'max:255'],
        ...
        $exists = ExamSet::query()->where('exam_id', $validated['exam_id'])->where('type', $validated['type'])->where('set_number', $validated['set_number'])->exists();

        if ($exists) {
            return back()->withErrors(['set_number' => 'This set number already exists for the selected exam and type.'])->withInput();
        }

        if ((float) $validated['pass_marks'] > (float) $validated['total_marks']) {
            return back()->withErrors(['pass_marks' => 'Pass marks cannot be greater than total marks.'])->withInput();
        }
        ...
        return redirect()->route('exam-sets.index')->with('success', 'Exam set created successfully.');
    }
    ...
    {
        $examSet->load(['exam.course', 'exam.batch']);
    ...
    {
        $exams = Exam::query()->with(['course', 'batch'])->where(function ($query) use ($examSet) {
            $query->where('is_active', true)->orWhere('id', $examSet->exam_id);
        })->whereNull('deleted_at')->orderByDesc('id')->get();

        return view('exam-sets.edit', compact('examSet', 'exams'));
    }
    ...
    public function update(Request $request, ExamSet $examSet)
    {
        ...
        $validated = $request->validate([
            'exam_id' => ['required', Rule::exists('exams', 'id')->whereNull('deleted_at')],
            'name' => ['required', 'string', 'max:100'],
            'type' => ['required', Rule::in(['mid', 'monthly'])],
            'question_type' => ['required', Rule::in(['mcq', 'evidence'])],
            'mode' => ['required', Rule::in(['online', 'offline'])],
        ...
        $exists = ExamSet::query()->where('exam_id', $validated['exam_id'])->where('type', $validated['type'])->where('set_number', $validated['set_number'])->where('id', '!=', $examSet->id)->exists();
        ...
        return redirect()->route('exam-sets.index')->with('success', 'Exam set updated successfully.');
    }
    ...
    public function destroy(ExamSet $examSet)
    {
        ...
        return redirect()->route('exam-sets.index')->with('success', 'Exam set deleted successfully.');
    }
